...
taken as the start, and nearby QSOs are snapped to a shared burst start so
...
def burst_starts(segs: list[Segment]) -> list[float]:
    """audio_t of the real start of every burst -- see `_bursts` for how one
    is delimited and `_tx_start` for how its TX-initiated start is found.
...
def _snap_to_burst(t: float, starts: list[float]) -> float:
    """The burst that produced the EDI-derived approximate time `t`.

    A QSO's own over necessarily starts at or before its (possibly
    minute-truncated) logged completion time, so this is the *latest* burst
    start <= t -- not simply the nearest one, which can jump ahead to the
    *next* contact's burst if the current QSO took a while (calling, retries)
    to complete before being logged.

    If no burst is <= t -- e.g. a QSO logged before any CW was ever
    ...
    the *first* burst in the whole recording here was a real bug: it could
    pull an early QSO's panel minutes into the future."""
    candidates = [c for c in starts if c <= t]
...
def qso_windows(
    ...
    snapped onto the actual WAV segment/burst boundary (see burst_starts)
    ...
    anchor into _snap_to_burst in place of the EDI's minute-precision
    ...
    exchange -- are one burst as far as burst_starts is concerned, since
    there's no audio structure to tell their overs apart at all. A QSO
    that snaps to the *same* burst as the previous QSO instead starts
    ...
    bursts = burst_starts(segs)
    starts: list[float] = []
    finishes: list[float | None] = []
    prev_burst: float | None = None
    for i, q in enumerate(qsos):
        ...
        snapped = _snap_to_burst(anchor_t, bursts)
        if (
            precise is not None
            and snapped == prev_burst
